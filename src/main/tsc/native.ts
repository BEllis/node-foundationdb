...
let mod = null
/*
let fdb_lib_path = process.env.FDB_LIB_PATH;
let fdb_include_path = process.env.FDB_INCLUDE_PATH;
let fdblib_use_installed = process.env.fdblib_use_installed ? true : false;
if (!fdb_lib_path && !fdb_include_path && !fdblib_use_installed) { */
  mod = require('../../bin/fdblib');
/*
} else {
  ...
  if (!fdb_include_path) {
    fdb_include_path = '/usr/local/include'
    if (process.platform == 'win32') {
      fdb_include_path=process.env.FOUNDATIONDB_INSTALL_PATH + '\\include'
    }
  }

  if (!fdb_include_path) {
    fdb_include_path = '/usr/local/lib'
    if (process.platform == 'win32') {
      fdb_include_path=process.env.FOUNDATIONDB_INSTALL_PATH + '\\lib\\foundationdb'
    } 
  }

  process.env.FDB_LIB_PATH = fdb_lib_path
  process.env.FDB_INCLUDE_PATH = fdb_include_path;
